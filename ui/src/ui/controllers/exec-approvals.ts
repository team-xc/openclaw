...
export async function loadExecApprovals(
  ...
  try {
    ...
    if (!rpc) {
      state.lastError = t("nodes.approvals.selectNodeBeforeLoad");
      return;
    }
...
export async function saveExecApprovals(
  ...
  try {
    ...
    if (!baseHash) {
      state.lastError = t("nodes.approvals.hashMissing");
      return;
    }
    ...
    if (!rpc) {
      state.lastError = t("nodes.approvals.selectNodeBeforeSave");
      return;
    }
