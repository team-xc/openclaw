...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const role = req.role?.trim()
    ? t("nodes.roleWithValue", { value: req.role })
    : t("nodes.roleUnknown");
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ${t("nodes.requestedAt", { role, age, repair })}
        </div>
      </div>
      <div class="list-meta">
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? t("nodes.status.revoked") : t("nodes.status.active");
  ...
  return html`
    <div class="row" style="justify-content: space-between; gap: 8px;">
      <div class="list-sub">${t("nodes.tokenSummary", { role: token.role, status, scopes, when })}</div>
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          ...
          <div class="card-sub">
            ${t("nodes.binding.subtitle")} <span class="mono">exec host=node</span>.
          </div>
        </div>
        <button
          class="btn"
        ...
        state.formMode === "raw"
          ? html`
              <div class="callout warn" style="margin-top: 12px">
                ${t("nodes.binding.rawModePrefix")} <strong>${t("nodes.binding.rawModeForm")}</strong> ${t("nodes.binding.rawModeSuffix")}
              </div>
            `
          : nothing
      }
          ...
          ${
            bindingValue === "__default__"
              ? t("nodes.binding.usesDefault", {
                  value: state.defaultBinding ?? t("nodes.binding.anyNodeLower"),
                })
