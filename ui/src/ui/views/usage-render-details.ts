  ...
  return html`
    ...
    <div class="session-summary-grid">
      <div class="session-summary-card">
        ...
        <div class="session-summary-meta">${t("usage.summary.userAssistant", { user: String(usage.messageCounts?.user ?? 0), assistant: String(usage.messageCounts?.assistant ?? 0) })}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.detail.toolCalls")}</div>
        <div class="session-summary-value">${toolCallCount}</div>
        <div class="session-summary-meta">${t("usage.summary.toolsUsed", { count: String(uniqueToolCount) })}</div>
      </div>
      <div class="session-summary-card">
        ...
        <div class="session-summary-meta">${t("usage.summary.toolResults", { count: String(usage.messageCounts?.toolResults ?? 0) })}</div>
      </div>
      <div class="session-summary-card">
...
function renderSessionDetailPanel(
  ...
  const cursorIndicator = filteredUsage ? t("usage.detail.filteredSuffix") : "";
      ...
      <div class="session-detail-header">
        ...
        <div class="session-detail-stats">
          ${
            usage
              ? html`
            <span><strong>${formatTokens(headerStats.totalTokens)}</strong> ${t("usage.common.tokens")}${cursorIndicator}</span>
            <span><strong>${formatCost(headerStats.totalCost)}</strong>${cursorIndicator}</span>
          `
              : nothing
          }
        </div>
        <button class="session-close-btn" @click=${onClose} title=${t("usage.detail.close")}>×</button>
      </div>
...
function renderTimeSeriesCompact(
  ...
  if (!timeSeries || timeSeries.points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.detail.noTimelineData")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="session-timeseries-compact">
      <div class="timeseries-header-row">
        <div class="card-title" style="font-size: 12px; color: var(--text);">${t("usage.detail.usageOverTime")}</div>
        <div class="timeseries-controls">
          ${
            hasSelection
              ? html`
            <div class="chart-toggle small">
              <button class="toggle-btn active" @click=${() => onCursorRangeChange?.(null, null)}>${t("usage.detail.reset")}</button>
            </div>
          `
              : nothing
          }
        ...
        >
          ...
          ${points.map((p, i) => {
            ...
            const tooltipLines = [
              ...
              `${formatTokens(val)} ${t("usage.common.tokens")}`,
            ];
            if (breakdownByType) {
              tooltipLines.push(`${t("usage.breakdown.output")} ${formatTokens(p.output)}`);
              tooltipLines.push(`${t("usage.breakdown.input")} ${formatTokens(p.input)}`);
              tooltipLines.push(`${t("usage.breakdown.cacheWrite")} ${formatTokens(p.cacheWrite)}`);
              tooltipLines.push(`${t("usage.breakdown.cacheRead")} ${formatTokens(p.cacheRead)}`);
            }
      ...
      <div class="timeseries-summary">
        ${
          hasSelection
            ? html`
              ${t("usage.detail.turnsSummary", {
                turns: `▶ ${t("usage.detail.turns", { start: String(rangeStartIdx + 1), end: String(rangeEndIdx), total: String(points.length) })}`,
                range: `${new Date(rangeStartTs).toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}–${new Date(rangeEndTs).toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}`,
                tokens: formatTokens(
                  filteredOutput + filteredInput + filteredCacheRead + filteredCacheWrite,
                ),
                cost: formatCost(filteredPoints.reduce((s, p) => s + (p.cost || 0), 0)),
              })}
            `
            : html`${t("usage.detail.messagesSummary", { count: String(points.length), tokens: formatTokens(cumTokens), cost: formatCost(cumCost) })}`
        }
      </div>
      ${
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                <div class="card-title" style="font-size: 12px; margin-bottom: 6px; color: var(--text);">${t("usage.detail.tokensByType")}</div>
...
function renderContextPanel(
  ...
  if (!contextWeight) {
    return html`
      <div class="context-details-panel">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.detail.noContextData")}</div>
      </div>
    `;
  }
  ...
  if (usage && usage.totalTokens > 0) {
    ...
    if (inputTokens > 0) {
      contextPct = t("usage.detail.inputShare", {
        value: Math.min((totalContextTokens / inputTokens) * 100, 100).toFixed(0),
      });
    }
  }
  ...
  return html`
    <div class="context-details-panel">
      <div class="context-breakdown-header">
        <div class="card-title" style="font-size: 12px; color: var(--text);">${t("usage.detail.systemPromptBreakdown")}</div>
        ${
          hasMore
      ...
      <p class="context-weight-desc">
        ${contextPct || t("usage.detail.baseContextPerMessage")}
      </p>
      <div class="context-stacked-bar">
        <div class="context-segment system" style="width: ${pct(systemTokens, totalContextTokens).toFixed(1)}%" title=${t("usage.detail.system") + `: ~${formatTokens(systemTokens)}`}></div>
        <div class="context-segment skills" style="width: ${pct(skillsTokens, totalContextTokens).toFixed(1)}%" title=${t("usage.detail.skills") + `: ~${formatTokens(skillsTokens)}`}></div>
        <div class="context-segment tools" style="width: ${pct(toolsTokens, totalContextTokens).toFixed(1)}%" title=${t("usage.detail.tools") + `: ~${formatTokens(toolsTokens)}`}></div>
        <div class="context-segment files" style="width: ${pct(filesTokens, totalContextTokens).toFixed(1)}%" title=${t("usage.detail.files") + `: ~${formatTokens(filesTokens)}`}></div>
      </div>
      <div class="context-legend">
        <span class="legend-item"><span class="legend-dot system"></span>${t("usage.detail.sys")} ~${formatTokens(systemTokens)}</span>
        <span class="legend-item"><span class="legend-dot skills"></span>${t("usage.detail.skills")} ~${formatTokens(skillsTokens)}</span>
        <span class="legend-item"><span class="legend-dot tools"></span>${t("usage.detail.tools")} ~${formatTokens(toolsTokens)}</span>
        <span class="legend-item"><span class="legend-dot files"></span>${t("usage.detail.files")} ~${formatTokens(filesTokens)}</span>
      </div>
      <div class="context-total">${t("usage.detail.totalApprox", { value: formatTokens(totalContextTokens) })}</div>
            ...
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.detail.skills")} (${skillsList.length})</div>
                    ...
                    ${
                      more > 0
                        ? html`<div class="context-breakdown-more">${t("usage.detail.more", { count: String(more) })}</div>`
                        : nothing
                    }
                  </div>
                `;
        ...
        ${
          toolsList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.detail.tools")} (${toolsList.length})</div>
                    <div class="context-breakdown-list">
                      ${toolsTop.map(
                        (t) => html`
                          <div class="context-breakdown-item">
        ...
        ${
          filesList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.detail.files")} (${filesList.length})</div>
                    <div class="context-breakdown-list">
                      ${filesTop.map(
                        (f) => html`
                          <div class="context-breakdown-item">
...
function renderSessionLogsCompact(
  ...
  const displayedCount =
    hasActiveFilters || hasCursorFilter
      ? t("usage.detail.displayedCount", {
          shown: String(filteredEntries.length),
          total: String(logs.length),
          suffix: hasCursorFilter ? t("usage.detail.timelineFiltered") : "",
        })
      : `${logs.length}`;
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>${t("usage.detail.conversation")} <span style="font-weight: normal; color: var(--muted);">${t("usage.detail.conversationCount", { count: displayedCount })}</span></span>
        <button class="btn btn-sm usage-action-btn usage-secondary-btn" @click=${onToggleExpandedAll}>
          ${expandedAll ? t("usage.detail.collapseAll") : t("usage.detail.expandAllLogs")}
        </button>
      </div>
      <div class="usage-filters-inline" style="margin: 10px 12px;">
        ...
        >
          <option value="user" ?selected=${roleSelected.has("user")}>${t("chat.labels.you")}</option>
          <option value="assistant" ?selected=${roleSelected.has("assistant")}>${t("chat.labels.assistant")}</option>
          <option value="tool" ?selected=${roleSelected.has("tool")}>${t("chat.labels.tool")}</option>
          <option value="toolResult" ?selected=${roleSelected.has("toolResult")}>${t("usage.detail.toolResult")}</option>
        </select>
        <select
          multiple
          size="4"
      ...
      <div class="session-logs-list">
        ...
        ${
          filteredEntries.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("usage.detail.noMessagesMatch")}</div>
              `
            : nothing
        }
      </div>
