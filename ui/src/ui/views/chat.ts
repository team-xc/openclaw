...
function renderFallbackIndicator(status: FallbackIndicatorStatus | null | undefined) {
  ...
  const details = [
    t("chat.fallback.details.selected", { value: status.selected }),
    phase === "cleared"
      ? t("chat.fallback.details.active", { value: status.selected })
      : t("chat.fallback.details.active", { value: status.active }),
    phase === "cleared" && status.previous
      ? t("chat.fallback.details.previous", { value: status.previous })
      : null,
    status.reason ? t("chat.fallback.details.reason", { value: status.reason }) : null,
    status.attempts.length > 0
      ? t("chat.fallback.details.attempts", { value: status.attempts.slice(0, 3).join(" | ") })
      : null,
  ]
    .filter(Boolean)
    .join(" • ");
...
export function renderChat(props: ChatProps) {
  ...
  const composePlaceholder = props.connected
    ? hasAttachments
      ? t("chat.compose.placeholderWithImages")
      : t("chat.compose.placeholder")
    : t("chat.compose.disconnectedPlaceholder");
