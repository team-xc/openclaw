...
const TOOL_SECTION_LABEL_KEYS: Record<string, string> = {
  fs: "agents.tools.sections.fs",
  runtime: "agents.tools.sections.runtime",
  web: "agents.tools.sections.web",
  memory: "agents.tools.sections.memory",
  sessions: "agents.tools.sections.sessions",
  ui: "agents.tools.sections.ui",
  messaging: "agents.tools.sections.messaging",
  automation: "agents.tools.sections.automation",
  nodes: "agents.tools.sections.nodes",
  agents: "agents.tools.sections.agents",
  media: "agents.tools.sections.media",
};

function localizeToolProfileLabel(id: string, fallback: string) {
    ...
    <section class="card">
        ...
        params.toolsCatalogError
          ? html`
              <div class="callout warn" style="margin-top: 12px">
                ${t("agents.tools.loadFallbackHint")}
              </div>
            `
          : nothing
      }
            ...
            html`
              <div class="agent-tools-section">
                <div class="agent-tools-header">
                  ${localizeToolSectionLabel(section.id, section.label)}
                  ${
                    "source" in section && section.source === "plugin"
                      ? html`
                          <span class="mono" style="margin-left: 6px">${t("agents.tools.plugin")}</span>
                        `
                      : nothing
                  }
                </div>
                        ...
                        <div>
                          <div class="agent-tool-title mono">
                            ...
                            ${
                              isOptional
                                ? html`
                                    <span class="mono" style="margin-left: 6px; opacity: 0.8">${t("common.optional")}</span>
                                  `
                                : nothing
                            }
                          </div>
      ...
      ${
        params.toolsCatalogLoading
          ? html`
              <div class="card-sub" style="margin-top: 10px">${t("agents.tools.refreshingCatalog")}</div>
            `
          : nothing
      }
    </section>
  ...
  return html`
    <section class="card">
      ...
      ${
        usingAllowlist
          ? html`
              <div class="callout info" style="margin-top: 12px">${t("agents.skills.customAllowlistHint")}</div>
            `
          : html`
              <div class="callout info" style="margin-top: 12px">
  ...
  return html`
    <div class="list-item agent-skill-row">
      <div class="list-main">
        ...
        ${
          missing.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skills.missing", { value: missing.join(", ") })}</div>`
            : nothing
        }
        ${
          reasons.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skills.reason", { value: reasons.join(", ") })}</div>`
            : nothing
        }
      </div>
      <div class="list-meta">
